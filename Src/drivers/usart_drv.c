...
void USART2_IRQHandler(void)
{
    if ((USART2->SR & USART_SR_IDLE) != 0U) {
        ...
        if (LineBuf_FindEnd(s_rx_buf, start, received) < received) {
            ...
            /* Disabling the stream mid-transfer also raises its TC flag. Left
             * set, DMA1_Stream5_IRQHandler would mistake it for a full buffer
             * and overwrite the length computed below with the buffer size.
             * Seen on the board: a 2-byte line was handed out as 64 bytes. */
            DMA1->HIFCR = DMA_HIFCR_CTCIF5 | DMA_HIFCR_CHTIF5 | DMA_HIFCR_CTEIF5 |
                          DMA_HIFCR_CDMEIF5 | DMA_HIFCR_CFEIF5;
