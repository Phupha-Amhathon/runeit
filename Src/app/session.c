 ...
 * it only has to differ between devices/keys.
 *
 * The device id and clock alone are predictable, so the ADC noise source is
 * XORed over them. XOR rather than replace: if the sensors are missing or fail
 * their health test the salt is no worse than the id/clock version, and if
 * they work it is genuinely unpredictable. */
static void MakeSalt(uint8_t salt[PARTITION_SALT_LEN])
{
    ...
    if (Entropy_Source_GetBytes(noise, sizeof(noise))) {
