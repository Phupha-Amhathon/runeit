...
## Not covered yet
AES is not implemented, so there is nothing to test. (`FIRST_MEET`/`MK_AUTH`/
`CHANGE_MK_MODE` and the panic button in the Stage C firmware are covered by
**Part 2**; `GENERATE_MODE` and the DMA ADC by **Part 3**.)
...
## G1.2 Serial flow and input validation
...
## G1.4 The save is encrypted at rest and toggles the partition (priority #1 and #2)
